...
const DEFS: Def[] = [
  ...
  { id: "knowledge_base", label: "Base de conhecimento (manuais do Drive)", description: "Pastas do Shared Drive (ex.: Formação/, Procedimentos/) sincronizadas à mão (botão «Sincronizar agora» na Base de conhecimento) para a base de conhecimento usada pelo tutor da formação, pelo assistente e pela pesquisa global; ficheiros carregados na app. Precisa do Shared Drive configurado (Definições → Comunicação → Google Drive).", require: [["GOOGLE_WORKSPACE_SERVICE_ACCOUNT_JSON", "GOOGLE_SERVICE_ACCOUNT_JSON"]], cron: "knowledge-sync", group: "main",
  ...
  { id: "cron", label: "Agendador (/api/cron/tick)", description: "Segredo partilhado com o cron-job.org e os workflows manuais do GitHub.", require: [["CRON_SECRET"]], group: "system", links: [{ label: "Estado do sistema", href: "/definicoes" }] },
