    ...
    <div className="space-y-6 max-w-[1240px]">
      ...
      <div className="flex items-center gap-2.5 bg-white border border-slate-200 rounded-full px-4 h-11 shadow-sm max-w-md">
        ...
        <input
          ...
          className="flex-1 bg-transparent outline-none border-none text-sm text-slate-900 placeholder:text-slate-500"
        />
      </div>
      ...
      {groups.map((g) => (
        <section key={g.id}>
          ...
          <div className="flex items-center gap-2.5 mb-3">
            ...
            <span className="text-xs text-muted-foreground">· {g.items.length} atalhos</span>
          </div>
      ...
      {groups.length === 0 && (
        <p className="text-center text-muted-foreground text-sm py-8">Nenhum módulo encontrado.</p>
      )}
    </div>
  );
}
