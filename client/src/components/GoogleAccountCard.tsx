...
// incremental (Gmail; Tarefas e Calendário ativam-se no cartão ao lado), só
// contas do Workspace da empresa.
    ...
    <div className={`bg-card border rounded-2xl shadow-sm ${needsReauth ? "border-amber-500/50" : "border-border"} ${compact ? "p-3" : "p-4"} space-y-2`}>
      ...
      {s?.connected && s.connectedAt && !compact && (
        <p className="text-[11.5px] text-muted-foreground">
          Ligada desde {fmtPTDateTime(s.connectedAt)} · Acessos: {s.features.filter((f) => f.granted).map((f) => GOOGLE_FEATURE_LABELS[f.id as GoogleFeature] ?? f.id).join(", ") || "—"}
        </p>
      )}
