...
export function renderNotificacoesDoc(): string {
  ...
  out.push("4. **Cidades.** Papéis de cidade (team leader, supervisor…) só recebem da(s) sua(s) cidade(s): centro de custos + cidades dadas em Permissões. O **super admin** recebe de todas. Papéis nacionais (frontoffice, backoffice, admin) recebem **só da própria cidade** por omissão (\"Papéis nacionais só da própria cidade\" ligado); o super admin pode desligá-lo por papel para receberem de todas. Quem tem \"todas as cidades\" (grupo Multipark) continua a receber de todas. Aviso sem cidade conhecida → só quem vê todas as cidades.");
